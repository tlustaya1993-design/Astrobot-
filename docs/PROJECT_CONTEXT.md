...
- [`CURRENT_STATE.md`](./CURRENT_STATE.md) — деплой Railway, открытые риски, z-index, что уточнить у владельца.
- [`DECISIONS.md`](./DECISIONS.md) — зафиксированные архитектурные решения (сессия, биллинг, streaming, prompt).
- [`STREAMING_NAVIGATION_NOTE.md`](./STREAMING_NAVIGATION_NOTE.md) — черновик PR: стрим без abort при навигации.
- Устаревшие детали в [`replit.md`](../replit.md) (**Replit не используется в prod**).
- Источник контрактов API: [`lib/api-spec/openapi.yaml`](../lib/api-spec/openapi.yaml).
...
**Прод:** деплой на **Railway** из ветки `main` ([`railway.toml`](../railway.toml)). Один HTTP-сервис: API + статика фронта. Подробности — [`CURRENT_STATE.md`](./CURRENT_STATE.md).
...
1. **Replit:** файлы `.replit` / `artifact.toml` в репозитории — **legacy**, prod только Railway.
2. **`replit.md`** удобен как обзор, но **устаревает** по части LLM (в коде приоритет Anthropic для чата).
3. **Системный промпт:** изменения только после audit — см. [`DECISIONS.md`](./DECISIONS.md) D-012.
4. **Стриминг при смене чата:** см. [`STREAMING_NAVIGATION_NOTE.md`](./STREAMING_NAVIGATION_NOTE.md).
5. **Сессия и контакт:** если в БД у диалога висит `contact_id`, а пользователь в UI переключился на «Я», возможна рассинхронизация до следующего сообщения с явной привязкой — логика исправляется на сервере/клиенте осторожно.
6. **`FRONTEND_DIST` и CWD** на Railway: см. комментарии в `app.ts`.
7. **ЮKassa:** без `YOOKASSA_*` создание платежа упадёт; Redis для billing опционален.
8. **Секреты:** не коммитить `.env`; в документации только имена переменных.
