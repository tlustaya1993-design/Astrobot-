...
- Интерактивы: видимый **focus** (кольцо/контраст), для иконок-кнопок — **`aria-label`** там, где нет текста.
- Контраст текста и кнопок — в духе **WCAG AA** на типичных фонах (`bg-card`, градиенты кнопок).
- Не полагаться только на цвет для состояния (дублировать подписью или иконкой).
