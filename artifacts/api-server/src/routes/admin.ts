...
function resolveEffectiveEmail(req: { authEmail?: string }): string | null {
  return req.authEmail?.trim().toLowerCase() ?? null;
}
...
async function requireAdmin(
  ...
  const effectiveEmail = resolveEffectiveEmail(req);
...
router.get("/me", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  res.json({ ok: true, email: resolveEffectiveEmail(req) });
});
    ...
    if ((!payment.webhookVerified || payment.status !== "succeeded") && payment.providerPaymentId) {
      try {
        ...
        if (providerPayment?.status) {
          const verification = verifyYooKassaPaymentForCredits(payment, providerPayment);
          await db
            .update(paymentsTable)
            .set({
              status: verification.ok
                ? "succeeded"
                : statusForUnverifiedProviderPayment(providerPayment.status),
