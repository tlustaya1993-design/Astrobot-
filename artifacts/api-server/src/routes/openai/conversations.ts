...
function buildSystemPrompt(
  ...
  const usedSignalsBlock =
    usedSignals.length > 0
      ? `Аспекты которые уже были разобраны в этом диалоге — не упоминать ни в основном тексте ни в ✦-блоке как новый анализ. Если нужно сослаться на уже сказанное — только коротко «как мы уже смотрели» и сразу переходи на новый слой карты (прогрессии / соляр / натал / другая планета / другой дом).\n${usedSignals.map((s) => `— ${s}`).join("\n")}\n\nПоследний предложенный крючок: ${lastHookTopic || "не было"}\n`
      : "";
