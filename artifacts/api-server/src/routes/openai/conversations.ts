  ...
  try {
    await db
      ...
      .set({
        ...
        requestsBalance: sql`${usersTable.requestsBalance} + ${paidBalanceDebit}`,
        updatedAt: new Date(),
      })
...
router.post("/conversations/:id/messages", async (req, res) => {
  ...
  const paidBalanceDebit = Math.max(0, balanceBefore - nextBalance);

  chargedRequestCost = requestCost;
  chargedPaidBalanceDebit = paidBalanceDebit;
  ...
  await db
    ...
    .set({
      ...
      requestsBalance: sql`GREATEST(0, ${usersTable.requestsBalance} - ${paidBalanceDebit})`,
      updatedAt: new Date(),
    })
    .where(eq(usersTable.sessionId, sessionId));
...
Тема вопроса или крючка определяет, какие показатели использовать.
Если вопрос или крючок были про деньги — используй только финансовые показатели, дома, управителей и сигналы, относящиеся к деньгам, доходам, ресурсам, инвестициям и партнёрским финансам.
Если вопрос был про отношения — используй только показатели, относящиеся к отношениям, партнёрству и совместимости.
Если вопрос был про карьеру — используй только показатели, относящиеся к карьере, статусу, работе и профессиональной реализации.
Если вопрос был про здоровье — используй только показатели, относящиеся к здоровью и физическому состоянию.
Тема вопроса или крючка важнее выбранного метода анализа.
Не переключайся на полный общий разбор выбранного метода.
Разворачивай именно ту тему, которую обсуждает пользователь.
Это правило имеет более высокий приоритет, чем желание дать полный обзор соляра, прогрессий, натальной карты, синастрии или любого другого метода.
...
${contactAstroSection ? `\n${contactAstroSection}\n` : ""}`
  ...
  const synastryModeNote = contact
    ? contactExtendedMode
      ? `\nРЕЖИМ КОНТАКТА (расширенный): Активна пара ${user?.name || "Пользователь"} + ${contact.name}. У тебя есть натал и транзиты пользователя, натал и транзиты контакта (и расширенные слои по контакту), плюс синастрия. Отвечай на вопросы «что с ним сейчас», «как он», «что между нами», а также на прогноз и сценарий отношений — опираясь на переданные расчёты; не выдумывай позиции планет. Синастрия: аспект → что обычно заметно во взаимодействии → практический смысл; не «судьба», «карма», «предназначение», «главный ориентир жизни», «буквально» без наблюдаемого слоя.\n`
      : `\nРЕЖИМ КОНТАКТА (база): Активна пара ${user?.name || "Пользователь"} + ${contact.name}. У тебя есть полный расчёт карты пользователя, натал контакта + транзиты к наталу контакта, и синастрия. Отвечай на «что с ним сейчас», «в каком он состоянии», «что между нами» — без долгосрочного прогноза и без соляра/прогрессий по контакту, пока не передан расширенный слой. Синастрия: аспект → наблюдаемое во взаимодействии → практический смысл; без пафоса «судьба/карма/ориентир жизни».\n`
    : "";
