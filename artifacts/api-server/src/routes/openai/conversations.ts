...
router.post("/conversations/:id/messages", async (req, res) => {
  ...
  const billingIdentityEmail = getVerifiedAccountEmail(owner.email, req.authEmail);
  const isUnlimited = isUnlimitedUser(billingIdentityEmail);

  if (!canAffordRequest(usedBeforeCharge, balanceBefore, requestCost, billingIdentityEmail)) {
    logger.info(
  ...
  const tier = detectTier(billingIdentityEmail, balanceBefore, isUnlimited);
