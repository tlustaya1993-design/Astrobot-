...
/** Нормализация для дедупа: без служебных слов и с унификацией символов аспектов. */
function signalKey(signal: string): string {
  return signal
    .toLowerCase()
    .replace(/транзитн[а-яё]*/gi, "")
    .replace(/натальн[а-яё]*/gi, "")
    .replace(/прогрессированн[а-яё]*/gi, "")
    .replace(/[△▲]/g, "трин")
    .replace(/⚹/g, "секстиль")
    .replace(/□/g, "квадрат")
    .replace(/☍/g, "оппозиция")
