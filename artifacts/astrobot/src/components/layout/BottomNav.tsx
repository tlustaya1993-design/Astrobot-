...
interface BottomNavProps {
  ...
  onChatsClick: () => void;
  onProfileClick: () => void;
  /** Pulse ring on Чаты button — used by onboarding step1 */
  highlightChats?: boolean;
}

export default function BottomNav({ activeTab, onChatsClick, onProfileClick, highlightChats }: BottomNavProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[60] bg-card/95 backdrop-blur-xl border-t border-border">
      <div className="flex">
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center gap-1 pt-3 pb-3 transition-colors touch-manipulation rounded-none ${
            activeTab === 'chats' ? 'text-primary' : 'text-muted-foreground'
          } ${highlightChats ? 'ring-2 ring-primary ring-inset animate-pulse' : ''}`}
        >
